...
public partial class Edit : IDisposable
{
    ...
    public ZonaDespachoEditarDto ZonaDespachoEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(ZonaDespachoAcceso.Editar))
            {
                INavigation.NavigateTo("zonas-despacho");
                Notify.Show("No tiene permiso para editar registros de [Zonas de Despacho]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("zonas-despacho");
                Notify.Show($"El registro de la [Zona de Despacho] consultado a editar no existe", "error");
                return;
            }

            ZonaDespachoEditar = IMapper.Map<ZonaDespachoEditarDto>(ZonaDespachoObtener);
            EditContext = new EditContext(ZonaDespachoEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IZonaDespacho.Editar((Guid) Id, ZonaDespachoEditar);

            IsModified = false;
            Notify.Show("La zona de despacho ha sido editada con éxito", "success");
            INavigation.NavigateTo($"zonas-despacho/{Id}");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void Volver() => INavigation.NavigateTo($"zonas-despacho{(ReturnPage == "view" ? $"/{Id}" : "")}");
