...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<ZonaDespachoListarDto> ListaZonasDespacho { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(ZonaDespachoAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IZonaDespacho.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"La zona de despacho {CodigoRegistro} ha sido eliminada con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("zonas-despacho/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"zonas-despacho/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"zonas-despacho/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaZonasDespacho.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaZonasDespacho = await IZonaDespacho.Listar(); 
        IsInitGrid = true; 
    }
