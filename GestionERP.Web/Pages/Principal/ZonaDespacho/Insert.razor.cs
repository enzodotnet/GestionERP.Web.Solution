...
public partial class Insert : IDisposable
{
    ...
    public ZonaDespachoInsertarDto ZonaDespachoInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        { 
            ZonaDespachoInsertar = new ZonaDespachoInsertarDto();

            EditContext = new EditContext(ZonaDespachoInsertar);
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(ZonaDespachoAcceso.Insertar))
            {
                INavigation.NavigateTo("zonas-despacho");
                Notify.Show("No tiene permiso para insertar registros de [Zonas de Despacho]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await IZonaDespacho.Insertar(ZonaDespachoInsertar);
            
            IsModified = false;
            Notify.Show("La zona de despacho ha sido insertada con éxito", "success");
    ...
    private void ValueCodigoUpperChanged(object codigo) => ZonaDespachoInsertar.Codigo = codigo?.ToString().ToUpper().Trim();
