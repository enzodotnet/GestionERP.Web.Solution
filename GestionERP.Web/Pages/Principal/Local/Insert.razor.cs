...
public partial class Insert : IDisposable
{
    ...
    public LocalInsertarDto LocalInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EditContext = new EditContext(LocalInsertar);
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(LocalAcceso.Insertar))
            {
                INavigation.NavigateTo("locales");
                Notify.Show("No tiene permiso para insertar registros de [Locales]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await ILocal.Insertar(LocalInsertar);

            IsModified = false;
            Notify.Show("El local ha sido insertado con éxito", "success");
    ...
    private void ValueCodigoUpperChanged(object codigo) => LocalInsertar.Codigo = codigo?.ToString().ToUpper().Trim();

    private void Volver() => INavigation.NavigateTo("locales");
    ...
    private void CargarItemCatalogoProvinciaPorRegion(ProvinciaCatalogoPorRegionDto item)
    {
        ...
        {
            LocalInsertar.CodigoDistrito = null;
    ...
    {
        LocalInsertar.CodigoDistrito = item.CodigoDistrito;
        LocalObtener.NombreDistrito = item.NombreDistrito;
        EditContext.NotifyFieldChanged(EditContext.Field("CodigoDistrito"));
        IsModified = true;
    } 
    #endregion
