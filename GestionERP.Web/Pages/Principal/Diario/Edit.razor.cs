...
public partial class Edit : IDisposable
{
    ...
    public DiarioEditarDto DiarioEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(DiarioAcceso.Editar))
            {
                INavigation.NavigateTo("diarios");
                Notify.Show("No tiene permiso para editar registros de [Diarios]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("diarios");
                Notify.Show("El registro del [Diario] consultado a editar no existe", "error");
                return;
            }
            DiarioEditar = IMapper.Map<DiarioEditarDto>(DiarioObtener);

            EditContext = new EditContext(DiarioEditar);
            IsInitPage = true;
        } 
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IDiario.Editar((Guid) Id, DiarioEditar);
            
            IsModified = false;
            Notify.Show("El diario ha sido editado con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"diarios{(ReturnPage == "view" ? $"/{Id}" : "")}");
