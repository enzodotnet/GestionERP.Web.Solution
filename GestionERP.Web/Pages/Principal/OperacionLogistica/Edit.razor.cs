...
public partial class Edit : IDisposable
{
    ...
    public OperacionLogisticaEditarDto OperacionLogisticaEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(OperacionLogisticaAcceso.Editar))
            {
                INavigation.NavigateTo("operaciones-logisticas");
                Notify.Show("No tiene permiso para editar registros de [Operaciones Logísticas]", "error");
                return;
            }

            OperacionLogisticaObtener = await IOperacionLogistica.Obtener((Guid) Id); 
            if (OperacionLogisticaObtener is null)
            {
                INavigation.NavigateTo("operaciones-logisticas");
                Notify.Show($"El registro de la [Operación Logística] consultado a editar no existe", "error");
                return;
            }
            OperacionLogisticaEditar = IMapper.Map<OperacionLogisticaEditarDto>(OperacionLogisticaObtener);

            EditContext = new EditContext(OperacionLogisticaEditar);
            IsInitPage = true;
        } 
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IOperacionLogistica.Editar((Guid) Id, OperacionLogisticaEditar);

            IsModified = false;
            Notify.Show("La operación logística ha sido editada con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"operaciones-logisticas{(ReturnPage == "view" ? $"/{Id}" : "")}");

    #region Catalogos  
    private void CargarItemCatalogoTipoOperacionAlmacen(TipoOperacionAlmacenCatalogoDto item)
    {
        if (OperacionLogisticaEditar.CodigoTipoOperacionAlmacen != item.CodigoTipoOperacionAlmacen)
        {
            OperacionLogisticaEditar.CodigoTipoOperacionAlmacen = item.CodigoTipoOperacionAlmacen;
