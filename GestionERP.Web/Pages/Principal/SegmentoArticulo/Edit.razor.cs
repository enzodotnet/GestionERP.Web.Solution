...
public partial class Edit : IDisposable
{
    ...
    public SegmentoArticuloEditarDto SegmentoArticuloEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(SegmentoArticuloAcceso.Editar))
            {
                INavigation.NavigateTo("segmentos-articulo");
                Notify.Show("No tiene permiso para editar registros de [Segmentos de Artículo]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("segmentos-articulo");
                Notify.Show($"El registro del [Segmento de Artículo] consultado a editar no existe", "error");
                return;
            }

            SegmentoArticuloEditar = IMapper.Map<SegmentoArticuloEditarDto>(SegmentoArticuloObtener);
            EditContext = new EditContext(SegmentoArticuloEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await ISegmentoArticulo.Editar((Guid) Id, SegmentoArticuloEditar);
            
            IsModified = false;
            Notify.Show("El segmento de artículo ha sido editado con éxito", "success");
            INavigation.NavigateTo($"segmentos-articulo/{Id}");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void Volver() => INavigation.NavigateTo($"segmentos-articulo{(ReturnPage == "view" ? $"/{Id}" : "")}");
