...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<ProcesoDocumentoListarDto> ListaProcesosDocumento { get; set; }
    private bool IsInitGrid { get; set; }
    private string TituloIndex { get; set; }
    [Parameter][SupplyParameterFromQuery(Name = "codigoTipoDocumento")] public string CodigoTipoDocumento { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            TituloIndex = "Lista de tipos de procesos de documentos";
            CodigoTipoDocumento ??= "";

            if (CodigoTipoDocumento != "")
                TituloIndex += " por tipo de documento";
			...
			ListaProcesosDocumento = await IProcesoDocumento.Listar(CodigoTipoDocumento);
			IsInitGrid = true;
		}
        catch (Exception ex)
        {
    ...
    private void IrVer(Guid id) => INavigation.NavigateTo($"procesos-documento/{id}");
