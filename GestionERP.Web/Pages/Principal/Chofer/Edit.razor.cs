...
public partial class Edit : IDisposable
{
    ...
    public ChoferEditarDto ChoferEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(ChoferAcceso.Editar))
            {
                INavigation.NavigateTo("choferes");
                Notify.Show("No tiene permiso para editar registros de [Choferes]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("choferes");
                Notify.Show("El registro del [Chofer] consultado a editar no existe", "error");
                return;
            }
            ChoferEditar = IMapper.Map<ChoferEditarDto>(ChoferObtener);

            EditContext = new EditContext(ChoferEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IChofer.Editar((Guid) Id, ChoferEditar);
            
            IsModified = false;
            Notify.Show("El chofer ha sido editado con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"choferes{(ReturnPage == "view" ? $"/{Id}" : "")}");
