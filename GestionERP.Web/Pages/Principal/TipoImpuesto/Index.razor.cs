...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<TipoImpuestoListarDto> ListaTiposImpuesto { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(TipoImpuestoAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await ITipoImpuesto.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"El tipo de impuesto {CodigoRegistro} ha sido eliminado con éxito", "success"); 
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("tipos-impuesto/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"tipos-impuesto/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"tipos-impuesto/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaTiposImpuesto.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaTiposImpuesto = await ITipoImpuesto.Listar(); 
        IsInitGrid = true;
    }
