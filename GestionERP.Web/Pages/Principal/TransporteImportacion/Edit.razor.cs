...
public partial class Edit : IDisposable
{
    ...
    public TransporteImportacionEditarDto TransporteEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(TransporteImportacionAcceso.Editar))
            {
                INavigation.NavigateTo("transportes-importacion");
                Notify.Show("No tiene permiso para editar registros de [Transportes]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("transportes-importacion");
                Notify.Show("El registro del [Transporte] consultado a editar no existe", "error");
                return;
            }
            TransporteEditar = IMapper.Map<TransporteImportacionEditarDto>(TransporteObtener);

            EditContext = new EditContext(TransporteEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await ITransporte.Editar((Guid) Id, TransporteEditar);

            IsModified = false;
            Notify.Show("El transporte ha sido editado con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"transportes-importacion{(ReturnPage == "view" ? $"/{Id}" : "")}");
