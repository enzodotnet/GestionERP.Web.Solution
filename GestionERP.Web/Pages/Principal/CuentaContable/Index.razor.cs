...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<CuentaContableListarDto> ListaCuentasContables { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(CuentaContableAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await ICuentaContable.Eliminar((Guid) RegistroId);
            await Listar();
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("cuentas-contables/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"cuentas-contables/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"cuentas-contables/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaCuentasContables.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaCuentasContables = await ICuentaContable.Listar(); 
        IsInitGrid = true; 
    }
