...
public partial class View : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (DocumentoObtener is null)
            {
                ...
                Notify.Show("El registro del [Documento] consultado a visualizar no está disponible", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IDocumento.Eliminar((Guid) Id);
            IsLoadingAction = false;
            INavigation.NavigateTo("documentos");
            Notify.Show($"El documento {DocumentoObtener.Codigo.Trim()} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrEditar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"documentos/{Id}/editar", new Dictionary<string, object> { ["returnpage"] = "view" }));
