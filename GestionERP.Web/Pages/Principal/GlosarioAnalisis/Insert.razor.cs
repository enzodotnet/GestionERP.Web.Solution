...
public partial class Insert : IDisposable
{
    ...
    public GlosarioAnalisisInsertarDto GlosarioAnalisisInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(GlosarioAnalisisAcceso.Insertar))
            {
                INavigation.NavigateTo("glosarios-analisis");
                Notify.Show("No tiene permiso para insertar registros de [Glosarios de Análisis]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await IGlosarioAnalisis.Insertar(GlosarioAnalisisInsertar);

            IsModified = false;
            Notify.Show("El glosario de análisis ha sido insertado con éxito", "success");
