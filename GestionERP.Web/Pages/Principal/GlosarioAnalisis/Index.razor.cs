...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<GlosarioAnalisisListarDto> ListaGlosariosAnalisis { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(GlosarioAnalisisAcceso.Insertar);

            if (!await IPermiso.ConsultaEsAsignadoPorSesion(GlosarioAnalisisAcceso.VerRegistros))
            {
                INavigation.NavigateTo("inicio");
                Notify.Show("No tiene permiso para ver registros del servicio principal de [Glosarios de Analisis]", "error");
                return;
            }
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IGlosarioAnalisis.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"El glosario de analisis {CodigoRegistro} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("glosarios-analisis/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"glosarios-analisis/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"glosarios-analisis/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaGlosariosAnalisis.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaGlosariosAnalisis = await IGlosarioAnalisis.Listar();
        IsInitGrid = true;
    }
