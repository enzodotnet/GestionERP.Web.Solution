...
public partial class View : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(GrupoDevengoAcceso.VerRegistros))
            {
                ...
                Notify.Show("No tiene permiso para ver registros del servicio principal de [Grupos de Devengos]", "error");
                return;
            }
            ...
            if (GrupoDevengoObtener is null)
            {
                ...
                Notify.Show("El registro del [Grupo de Devengo] consultado a visualizar no está disponible", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IGrupoDevengo.Eliminar((Guid) Id);
            IsLoadingAction = false;
            INavigation.NavigateTo("grupos-devengo");
            Notify.Show($"El cargo {GrupoDevengoObtener.Codigo.Trim()} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrEditar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"grupos-devengo/{Id}/editar", new Dictionary<string, object> { ["returnpage"] = "view" }));
