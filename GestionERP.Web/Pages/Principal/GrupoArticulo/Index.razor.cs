...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<GrupoArticuloListarDto> ListaGruposArticulo { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(GrupoArticuloAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IGrupoArticulo.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"El grupo de artículo {CodigoRegistro} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("grupos-articulo/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"grupos-articulo/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"grupos-articulo/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaGruposArticulo.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaGruposArticulo = await IGrupoArticulo.Listar(); 
        IsInitGrid = true; 
    }
