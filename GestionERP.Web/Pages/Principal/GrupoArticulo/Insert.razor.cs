...
public partial class Insert : IDisposable
{
    ...
    public GrupoArticuloInsertarDto GrupoArticuloInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        { 
            GrupoArticuloInsertar = new GrupoArticuloInsertarDto();

            EditContext = new EditContext(GrupoArticuloInsertar);
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(GrupoArticuloAcceso.Insertar))
            {
                INavigation.NavigateTo("grupos-articulo");
                Notify.Show("No tiene permiso para insertar registros de [Grupos de Artículo]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await IGrupoArticulo.Insertar(GrupoArticuloInsertar);
            
            IsModified = false;
            Notify.Show("El grupo de artículo ha sido insertado con éxito", "success");
