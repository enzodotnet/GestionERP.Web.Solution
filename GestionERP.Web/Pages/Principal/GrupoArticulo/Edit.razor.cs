...
public partial class Edit : IDisposable
{
    ...
    public GrupoArticuloEditarDto ClaseEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(GrupoArticuloAcceso.Editar))
            {
                INavigation.NavigateTo("grupos-articulo");
                Notify.Show("No tiene permiso para editar registros de [Grupos de Artículo]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("grupos-articulo");
                Notify.Show($"El registro de la [Grupo de Artículo] consultado a editar no existe", "error");
                return;
            }

            ClaseEditar = IMapper.Map<GrupoArticuloEditarDto>(GrupoArticuloObtener);
            EditContext = new EditContext(ClaseEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IGrupoArticulo.Editar((Guid) Id, ClaseEditar);
            
            IsModified = false;
            Notify.Show("El grupo de artículo ha sido editado con éxito", "success");
            INavigation.NavigateTo($"grupos-articulo/{Id}");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void Volver() => INavigation.NavigateTo($"grupos-articulo{(ReturnPage == "view" ? $"/{Id}" : "")}");
