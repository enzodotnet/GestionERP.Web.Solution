...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<TipoProvisionListarDto> ListaTiposProvision { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(TipoProvisionAcceso.VerRegistros))
            {
                ...
                Notify.Show("No tiene permiso para ver registros del servicio principal de [Típos de provisión]", "error");
                return;
            }

			ListaTiposProvision = await ITipoProvision.Listar();
			IsInitGrid = true;
		}
        catch (Exception ex)
        {
    ...
    private void IrVer(Guid id) => INavigation.NavigateTo($"tipos-provision/{id}");
