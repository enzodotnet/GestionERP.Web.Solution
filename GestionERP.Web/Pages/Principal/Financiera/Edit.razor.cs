...
public partial class Edit : IDisposable
{
    ...
    public FinancieraEditarDto FinancieraEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(FinancieraAcceso.Editar))
            {
                INavigation.NavigateTo("financieras");
                Notify.Show("No tiene permiso para editar registros de [Financieras]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("financieras");
                Notify.Show($"El registro de la [Financiera] consultado a editar no existe", "error");
                return;
            }
            FinancieraEditar = IMapper.Map<FinancieraEditarDto>(FinancieraObtener);

            EditContext = new EditContext(FinancieraEditar);
            IsInitPage = true;
        } 
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IFinanciera.Editar((Guid) Id, FinancieraEditar);
            
            IsModified = false;
            Notify.Show("La financiera ha sido editada con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"financieras{(ReturnPage == "view" ? $"/{Id}" : "")}");

    #region Catalogos 
    private void CargarItemCatalogoTipoFinanciera(TipoFinancieraCatalogoDto item)
    {
        if (string.IsNullOrEmpty(FinancieraEditar.CodigoTipoFinanciera) || FinancieraEditar.CodigoTipoFinanciera != item.CodigoTipoFinanciera)
        {
            FinancieraEditar.CodigoTipoFinanciera = item.CodigoTipoFinanciera;
