...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<FinancieraListarDto> ListaFinancieras { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(FinancieraAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IFinanciera.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"La financiera {CodigoRegistro} ha sido eliminada con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("financieras/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"financieras/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"financieras/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaFinancieras.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaFinancieras = await IFinanciera.Listar();
        IsInitGrid = true;
    }
