...
public partial class Edit : IDisposable
{
    ...
    public TipoGastoImportacionEditarDto TipoGastoImportacionEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            TipoGastoImportacionEditar = new();
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(TipoGastoImportacionAcceso.Editar))
            {
                INavigation.NavigateTo("tipos-gasto-importacion");
                Notify.Show("No tiene permiso para editar registros de [Tipos de Gasto de Importacion]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("tipos-gasto-importacion");
                Notify.Show("El registro del [Tipo de Gasto de Importacion] consultado a editar no existe", "error");
                return;
            }

            TipoGastoImportacionEditar = IMapper.Map<TipoGastoImportacionEditarDto>(TipoGastoImportacionObtener);
            EditContext = new EditContext(TipoGastoImportacionEditar);
            IsInitPage = true;
        }
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await ITipoGastoImportacion.Editar((Guid) Id, TipoGastoImportacionEditar);
            IsModified = false;
            Notify.Show("El tipo de gasto de importación ha sido editada con éxito", "success");
            INavigation.NavigateTo($"tipos-gasto-importacion/{Id}");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void Volver() => INavigation.NavigateTo($"tipos-gasto-importacion{(ReturnPage == "view" ? $"/{Id}" : "")}");
