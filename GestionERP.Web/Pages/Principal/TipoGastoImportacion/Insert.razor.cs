...
public partial class Insert : IDisposable
{
    ...
    public TipoGastoImportacionInsertarDto TipoGastoImportacionInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        { 
            TipoGastoImportacionInsertar = new TipoGastoImportacionInsertarDto();
            Tipos = TipoGastoImportacionTipoType.ObtenerTipos();

            EditContext = new EditContext(TipoGastoImportacionInsertar);
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(TipoGastoImportacionAcceso.Insertar))
            {
                INavigation.NavigateTo("tipos-gasto-importacion");
                Notify.Show("No tiene permiso para insertar registros de [Tipos de Gasto de Importacion]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await ITipoGastoImportacion.Insertar(TipoGastoImportacionInsertar);

            IsModified = false;
            Notify.Show("El tipo de gasto de importacion ha sido insertado con éxito", "success");
    ...
    private void ValueCodigoUpperChanged(object codigo) => TipoGastoImportacionInsertar.Codigo = codigo?.ToString().ToUpper().Trim();
