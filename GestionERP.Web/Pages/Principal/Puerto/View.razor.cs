...
public partial class View : IDisposable
{
    ...
    public PuertoObtenerDto PuertoObtener { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            PuertoObtener = await IPuerto.Obtener((Guid) Id);

            if (PuertoObtener is null)
            {
                INavigation.NavigateTo("puertos");
                Notify.Show("El registro del [Puerto] consultado a visualizar no está disponible", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private void Volver() => INavigation.NavigateTo("puertos");
