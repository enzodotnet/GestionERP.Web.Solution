...
public partial class Insert : IDisposable
{
    ...
    public ZonaVentaInsertarDto ZonaVentaInsertar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        { 
            ZonaVentaInsertar = new ZonaVentaInsertarDto();

            EditContext = new EditContext(ZonaVentaInsertar);
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(ZonaVentaAcceso.Insertar))
            {
                INavigation.NavigateTo("zonas-venta");
                Notify.Show("No tiene permiso para insertar registros de [Zonas de Venta]", "error");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            Guid id = await IZonaVenta.Insertar(ZonaVentaInsertar);

            IsModified = false;
            Notify.Show("La zona de venta ha sido insertada con éxito", "success");
    ...
    private void ValueCodigoUpperChanged(object codigo) => ZonaVentaInsertar.Codigo = codigo?.ToString().ToUpper().Trim();
