...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<ZonaVentaListarDto> ListaZonasVenta { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(ZonaVentaAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IZonaVenta.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"La zona de venta {CodigoRegistro} ha sido eliminada con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("zonas-venta/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"zonas-venta/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"zonas-venta/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaZonasVenta.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaZonasVenta = await IZonaVenta.Listar();
        IsInitGrid = true;
    }
