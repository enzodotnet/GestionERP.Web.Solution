...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<ArticuloListarDto> ListaArticulos { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(ArticuloAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IArticulo.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"El artículo {CodigoRegistro} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("articulos/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"articulos/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"articulos/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaArticulos.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaArticulos = await IArticulo.Listar(); 
        IsInitGrid = true; 
    }
