...
public partial class Edit : IDisposable
{
    ...
    public ArticuloEditarDto ArticuloEditar { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(ArticuloAcceso.Editar))
            {
                INavigation.NavigateTo("articulos");
                Notify.Show("No tiene permiso para editar registros de [Artículos]", "error");
                return;
            }
            ...
            {
                INavigation.NavigateTo("articulos");
                Notify.Show("El registro del [Artículo] consultado a editar no existe", "error");
                return;
            }
            ArticuloEditar = IMapper.Map<ArticuloEditarDto>(ArticuloObtener);

            EditContext = new EditContext(ArticuloEditar);
            IsInitPage = true;
        } 
        catch (Exception ex)
        {
    ...
    private async Task Editar()
    {
        try
        {
            ...
            await IArticulo.Editar((Guid) Id, ArticuloEditar);

            IsModified = false;
            Notify.Show("El artículo ha sido editado con éxito", "success");
    ...
    private void Volver() => INavigation.NavigateTo($"articulos{(ReturnPage == "view" ? $"/{Id}" : "")}");

    #region Catalogos
    private void CargarItemCatalogoFamiliaProducto(FamiliaProductoCatalogoDto item)
    {
        if (string.IsNullOrEmpty(ArticuloEditar.CodigoFamiliaProducto) || ArticuloEditar.CodigoFamiliaProducto != item.CodigoFamiliaProducto)
        {
            ArticuloEditar.CodigoFamiliaProducto = item.CodigoFamiliaProducto;
