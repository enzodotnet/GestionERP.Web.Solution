...
public partial class Index : IDisposable
{
    ...
    private IEnumerable<ServicioListarDto> ListaServicios { get; set; }
    ...
    [Parameter][SupplyParameterFromQuery(Name = "codigoModulo")] public string CodigoModulo { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        { 
            ...
            TituloIndex = "Lista de servicios";
            CodigoModulo ??= "";

            if (CodigoModulo != "")
                TituloIndex += " por módulo";
            ...
            EsAsignadoInsertar = await IPermiso.ConsultaEsAsignadoPorSesion(ServicioAcceso.Insertar);
    ...
    private async Task Eliminar()
    {
        try
        {
            ...
            await IServicio.Eliminar((Guid) RegistroId);
            await Listar();
 
            Notify.Show($"El servicio {CodigoRegistro} ha sido eliminado con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void IrInsertar() => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters("servicios/insertar", new Dictionary<string, object> { ["returnpage"] = "index" }));

    private void IrVer(Guid id) => INavigation.NavigateTo($"servicios/{id}");

    private void IrEditar(Guid id) => INavigation.NavigateTo(INavigation.GetUriWithQueryParameters($"servicios/{id}/editar", new Dictionary<string, object> { ["returnpage"] = "index" }));
    ...
    {
        RegistroId = id;
        CodigoRegistro = visible ? ListaServicios.Where(x => x.Id == (Guid) id).Select(x => x.Codigo).FirstOrDefault().Trim() : null;
        EsVisibleDialogEliminar = visible;
    }

    private async Task Listar()
    {
        ListaServicios = await IServicio.Listar(CodigoModulo);
        IsInitGrid = true;
    }
