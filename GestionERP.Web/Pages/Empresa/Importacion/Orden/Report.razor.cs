...
public partial class Report : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(OrdenAcceso.VerReporte, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para ver reporte de órdenes de importación", "error");
				return;
			}
            ...
            if (Orden is null)
            {
                ...
                Notify.Show($"El registro de la orden de importación consultado a ver reporte no está disponible", "error");
                return;
            }
            ReportSource = new
            (
    ...
    { 
        try
        {
            ...
            Notify.Show($"La orden de importacion {Orden.Codigo} ha sido impresa con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
