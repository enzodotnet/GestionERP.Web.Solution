...
public partial class UpdateState : IDisposable
{
    ...
    private bool EsVisibleListaNotas { get; set; }
    ...
    private async Task Accionar()
    {
        try
        {
            ...
            Notify.Show($"La orden de importación {CodigoRegistro} ha sido {ResultadoAccion} con éxito", "success");
        }
        catch (Exception ex)
        {
    ...
    private async Task IniciarVistaAccion()
    {
		try
		{
			...
			{
				INavigation.NavigateTo($"{rutaEmpresa}/inicio");
				Notify.Show("El tipo de acción al que intenta acceder para actualizar estado en órdenes de importación no es válido", "error");
				return;
			}
