...
public partial class View : IDisposable
{
    ...
    private const string nombreReporte = "SolicitudImportacionReport.trdp";
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (Solicitud is null)
            {
                ...
                Notify.Show($"El registro de la [Solicitud de Importacion] a visualizar no está disponible", "error");
                return;
            }
    ...
    private async Task Accionar()
    {
        try
        {
            ...
            Notify.Show($"La solicitud de importación {Solicitud.Codigo} ha sido {ResultadoAccion} con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private async Task ImprimirDirecto()
    {
        try
        {
            ...
            Notify.Show($"La solicitud de importación {Solicitud.Codigo} ha sido impresa con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
