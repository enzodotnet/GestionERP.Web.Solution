...
public partial class UpdateState : IDisposable
{
    ...
    private async Task Accionar()
    {
        try
        {
            ...
            Notify.Show($"La solicitud de compra {CodigoRegistro} ha sido {ResultadoAccion} con éxito", "success");
        }
        catch (Exception ex)
        {
    ...
    private async Task IniciarVistaAccion()
    {
		try
		{
			...
			{
				INavigation.NavigateTo($"{rutaEmpresa}/inicio");
				Notify.Show("El tipo de acción al que intenta acceder para actualizar estado en [Solicitudes de Importacions] no es válido", "error");
				return;
			} 
