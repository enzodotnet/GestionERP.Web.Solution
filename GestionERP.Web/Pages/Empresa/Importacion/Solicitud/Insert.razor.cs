...
public partial class Insert : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (!await IPermiso.ConsultaEsAsignadoPorSesion(SolicitudAcceso.Emitir, Empresa.Codigo))
            {
                ...
                Notify.Show("No tiene permiso para emitir registros de [Solicitudes de Importacion]", "error");
                return;
            }
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            IsModified = false;
            Notify.Show("La solicitud de importación ha sido emitida con éxito en la empresa", "success");
    ...
    private bool VerificarRegistroEsValido()
    {
        ...
        if (SolicitudInsertar.FechaEstimadaETD?.Date > SolicitudInsertar.FechaEstimadaETA?.Date)
        {  
            Fnc.MostrarAlerta(Alert, "La fecha estimada ETD debe ser menor o igual a la fecha estimada ETA", "error");
            esValido = false;
        }
		if (SolicitudInsertar.FechaEstimadaETA?.Date > SolicitudInsertar.FechaReposicionStock?.Date)
		{
        ...
        {
            Fnc.MostrarAlerta(Alert, "Es obligatorio que agregue ítem(s) al detalle de la solicitud de importación", "error");
            esValido = false;
    ...
    private void OnChangeDetalleCantidadHandler(object value) => IsModifiedDetalle = Fnc.VerifyContextIsChanged(Detalle.Cantidad != (decimal?)value, DetalleContext, "Cantidad");
	...
	{
		SolicitudInsertar.CodigoPaisOrigen = Solicitud.CodigoPaisOrigen = item.CodigoPais;
	...
	{
		SolicitudInsertar.CodigoPaisProcedencia = Solicitud.CodigoPaisProcedencia = item.CodigoPais;
	...
	private async Task OnChangeEntidadHandler(object value)
	{
		...
		{
			if ((Solicitud.CodigoEntidad ?? "").Trim() == codigo) goto exit;
    ...
    {
        SolicitudInsertar.CodigoPaisOrigen = value?.Trim().ToUpper();
        if (!string.IsNullOrEmpty(Validator.MsgErrorPaisOrigen) && (Solicitud.CodigoPaisOrigen ?? "").Trim() != (SolicitudInsertar.CodigoPaisOrigen ?? ""))
    ...
    private async Task OnChangePaisOrigenHandler(object value)
	{
		...
		{
			if ((Solicitud.CodigoPaisOrigen ?? "") == codigo) goto exit; 
    ...
    {
        SolicitudInsertar.CodigoPaisProcedencia = value?.Trim().ToUpper();
        if (!string.IsNullOrEmpty(Validator.MsgErrorPaisProcedencia) && (Solicitud.CodigoPaisProcedencia ?? "").Trim() != (SolicitudInsertar.CodigoPaisProcedencia ?? ""))
    ...
    private async Task OnChangePaisProcedenciaHandler(object value)
	{
		...
		{
			if ((Solicitud.CodigoPaisProcedencia ?? "") == codigo) goto exit;
	...
	private void OnChangeFechaEmisionHandler(object value) => IsModified = Fnc.VerifyContextIsChanged((value is null && Solicitud.FechaEmision.HasValue) || (!Solicitud.FechaEmision.HasValue && value is not null) || (Solicitud.FechaEmision.HasValue && value is not null && Solicitud.FechaEmision != (DateTime?)value), EditContext, "FechaEmision");
	#endregion
