...
public partial class Edit : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(OrdenAcceso.Editar, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para editar registros de [Órdenes de Servicio]", "error");
				return;
			}
            ...
            if (Orden is null || Orden.CodigoEstado is not "ED")
            {
                ...
                Notify.Show($"El registro de la [Orden de Servicio] consultado a editar no está disponible", "error");
                return;
            }
    ...
    private async Task Editar()
    {
        try
        {
            ...
            IsModified = false;
            Notify.Show($"La orden de servicio {Orden.Codigo} ha sido editada con éxito", "success");
    ...
    private bool VerificarRegistroEsValido()
    {
        ...
        {
            Fnc.MostrarAlerta(Alert, "Es obligatorio que agregue ítem(s) al detalle de la orden de servicio", "error");
            esValido = false;
        }
        else 
        {
            ...
            {
                Fnc.MostrarAlerta(Alert, "Está pendiente completar alguna celda del [precio unitario] del detalle de la orden de servicio", "error");
                esValido = false;
            }
        }
	...
	public void EditDetalleHandler(GridCommandEventArgs args) => IsEditingGridDetalle = args.Field is "Cantidad" or "PrecioUnitario";
    ...
    private void ModificarImportesItem(int? cantidad, decimal? precioUnitario, bool esAfectoImpuesto, int indexGrid = -1, int indexEdit = -1)
    {
    ...
    private void ModificarImportesTotales()
    {
        ...
        OrdenEditar.TotalImporteBruto = (decimal)(esGridVacio ? 0 : GridDetalles.Sum(x => x.ImporteBruto));
        OrdenEditar.TotalImporteImpuesto = (decimal)(esGridVacio ? 0 : GridDetalles.Sum(x => x.ImporteImpuesto));
        OrdenEditar.TotalImporteNeto = (decimal)(esGridVacio ? 0 : GridDetalles.Sum(x => x.ImporteNeto));
    }
            ...
            {  
			    ...
			    if (item is not null)
			    {
                    ...
                    if (Detalle.EsAfectoImpuesto != item.EsAfectoImpuesto)
                    {
					    ...
					    if (DetalleInsertar.Cantidad.HasValue && DetalleInsertar.PrecioUnitario.HasValue)
    ...
    private async Task AccionarDetalle(string tipoAccion)
    {
        ...
        switch (tipoAccion)
        {
            ...
            case "E":
                ...
                GridDetalles[indexGrid] = ItemGridDetalle = IMapper.Map<OrdenDetalleGrid>(IMapper.Map(DetalleEditar, Detalle));
                break;
            case "Q" or "X":
                if (tipoAccion == "Q")
    ...
    {
        OrdenEditar.CodigoLocalAtencion = Orden.CodigoLocalAtencion = item.CodigoLocal;
        Orden.NombreLocalAtencion = item.NombreLocal;
        IsChangedLocal = true;
    ...
    {
        OrdenEditar.CodigoLocalAtencion = value?.Trim().ToUpper();
        if (!string.IsNullOrEmpty(Validator.MsgErrorLocalAtencion) && (Orden.CodigoLocalAtencion ?? "").Trim() != (OrdenEditar.CodigoLocalAtencion ?? ""))
            Orden.CodigoLocalAtencion = Validator.MsgErrorLocalAtencion = null;
    }
    ...
    private async Task OnChangeLocalHandler(object value)
    {
		...
		{ 
			OrdenEditar.CodigoLocalAtencion = string.IsNullOrEmpty(codigo) ? null : codigo; 
            if ((Orden.CodigoLocalAtencion ?? "") != codigo)
			{
                IsChangedLocal = true;
                if (!string.IsNullOrEmpty(codigo))
                {
				    ...
				    if (item is not null)
				    {
					    Orden.CodigoLocalAtencion = item.CodigoLocal;
					    Orden.NombreLocalAtencion = item.NombreLocal; 
					    goto exit;
				    } 
                    EditContext.NotifyFieldChanged(EditContext.Field("CodigoLocalAtencion")); 
            ...
            {
                if (!IsChangedLocal) EditContext.MarkAsUnmodified(EditContext.Field("CodigoLocalAtencion"));
				goto exit;
			}
		}
        Orden.CodigoLocalAtencion = codigo;
		Orden.NombreLocalAtencion = null;
	exit:
		NotifyChange();
    }
    ...
    private void OnChangeDescripcionLugarAtencionHandler(object value) => NotifyChange(Fnc.VerifyContextIsChanged((Orden.DescripcionLugarAtencion ?? "") != ((string)value ?? ""), EditContext, "DescripcionLugarAtencion"));
	...
	private void OnChangeEsVinculableProduccionHandler(object value) => NotifyChange(Fnc.VerifyContextIsChanged(Orden.EsVinculableProduccion != (bool)value, EditContext, "EsVinculableProduccion"));
    ...
    private void OnChangeFechaAtencionHandler(object value) => NotifyChange(Fnc.VerifyContextIsChanged(Orden.FechaAtencion != (DateTime?)value, EditContext, "FechaAtencion"));
    #endregion
