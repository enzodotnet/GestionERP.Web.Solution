...
public partial class View : IDisposable
{
    ...
    public OrdenDetalleAceptacionObtenerDto AceptacionObtener { get; set; }
    ...
    public bool EsVisibleModalAceptaciones { get; set; }
    ...
    private IEnumerable<OrdenFlag> EstadosAnticipo { get; set; }
    private IEnumerable<OrdenFlag> EstadosProduccion { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            EstadosAnticipo = OrdenFlag.EstadosAnticipo();
            EstadosProduccion = OrdenFlag.EstadosProduccion();
            ...
            if (Orden is null)
            {
                ...
                Notify.Show($"El registro de la [Orden de Servicio] a visualizar no está disponible", "error");
                return;
            }
    ...
    private async Task Accionar()
    {
        try
        {
            ...
            Notify.Show($"La orden de servicio {Orden.Codigo} ha sido {ResultadoAccion} con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void MostrarProvisiones(OrdenDetalleAceptacionObtenerDto item)
    {
        AceptacionObtener = item;
        VisibleProvisionesChangedHandler(true);
    }

    private void MostrarAceptaciones(OrdenDetalleObtenerDto item)
    {
        Detalle = item;
        VisibleAceptacionesChangedHandler(true);
    }

    public void VisibleAceptacionesChangedHandler(bool esVisible) => EsVisibleModalAceptaciones = esVisible;
