...
public partial class Insert : IDisposable
{
    ...
    protected async Task IniciarVistaOrigen()
    {
        try
        {
            ...
            {
                INavigation.NavigateTo($"{rutaEmpresa}/inicio");
                Notify.Show("No tiene permiso para emitir registros de [Órdenes de Servicio]", "error");
                return;
            }
    ...
    private async Task Insertar()
    {
        try
        {
            ...
            IsModified = false;
            Notify.Show("La orden de servicio ha sido emitida con éxito en la empresa", "success");
    ...
    private bool VerificarRegistroEsValido()
    {
        ...
        else 
        {
            ...
            else
            {
                ...
                {
                    Fnc.MostrarAlerta(Alert, "Está pendiente completar alguna celda de [cantidad] del detalle de la orden de servicio", "error");
                    esValido = false;
                } 
	...
	private async Task AgregarItemsSolicitud()
    {
        ...
        {
            string codigoLocalAtencion = ItemsSelectedSolicitud.Select(x => x.CodigoLocalAtencion).First();
            if (string.IsNullOrEmpty(OrdenInsertar.CodigoLocalAtencion) && !string.IsNullOrEmpty(codigoLocalAtencion) && !ItemsSelectedSolicitud.Any(x => x.CodigoLocalAtencion != codigoLocalAtencion))
            {
                OrdenInsertar.CodigoLocalAtencion = Orden.CodigoLocalAtencion = codigoLocalAtencion;
                Orden.NombreLocalAtencion = ItemsSelectedSolicitud.Select(x => x.NombreLocalAtencion).First();
    ...
    private void ModificarImportesItem(int? cantidad, decimal? precioUnitario, bool esAfectoImpuesto, int indexGrid = -1)
    {
    ...
    { 
		...
		OrdenInsertar.CodigoLocalAtencion = CodigoLocalNumerador = item.CodigoLocal;
    ...
    {
        OrdenInsertar.CodigoLocalAtencion = Orden.CodigoLocalAtencion = item.CodigoLocal;
        Orden.NombreLocalAtencion = item.NombreLocal;
        Validator.MsgErrorLocalAtencion = null;
        EditContext.NotifyFieldChanged(EditContext.Field("CodigoLocalAtencion"));
        IsModified = EditContext.IsModified();
    ...
    {
        OrdenInsertar.CodigoLocalAtencion = value?.Trim().ToUpper();
        if (!string.IsNullOrEmpty(Validator.MsgErrorLocalAtencion) && (Orden.CodigoLocalAtencion ?? "").Trim() != (OrdenInsertar.CodigoLocalAtencion ?? ""))
    ...
    private async Task OnChangeLocalHandler(object value)
    {
		...
		{ 
			if (!string.IsNullOrEmpty(codigo))
			{ 
                if ((Orden.CodigoLocalAtencion ?? "") == codigo) goto exit;
            ...
            {
                OrdenInsertar.CodigoLocalAtencion = null;
    ...
    private async Task OnChangeCentroCostoHandler(object value)
	{
		...
		Orden.CodigoCentroCosto = Orden.NombreCentroCosto = null;
	exit:
		IsModified = EditContext.IsModified();
	}
    ...
    private void OnChangeDescripcionLugarAtencionHandler(object value) => IsModified = Fnc.VerifyContextIsChanged(!string.IsNullOrEmpty((string)value), EditContext, "DescripcionLugarAtencion");
	...
	private void OnChangeEsVinculableProduccionHandler(object value) => IsModified = Fnc.VerifyContextIsChanged((bool)value, EditContext, "EsVinculableProduccion");
    ...
    private void OnChangeFechaEmisionHandler(object value)
    {
        ...
        if (OrdenInsertar.FechaAtencion.HasValue)
