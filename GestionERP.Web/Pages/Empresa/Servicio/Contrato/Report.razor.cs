...
public partial class Report : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(ContratoAcceso.VerReporte, Empresa.Codigo))
			{
				INavigation.NavigateTo($"{rutaEmpresa}/inicio");
				Notify.Show("No tiene permiso para ver reporte de contratos", "error");
				return;
			}

			EsAsignadoImprimirDirecto = await IPermiso.ConsultaEsAsignadoPorSesion(ContratoAcceso.ImprimirDirecto, Empresa.Codigo);

            if (await IPermiso.ConsultaEsAsignadoPorSesion(ContratoAcceso.ExportarReportePDF, Empresa.Codigo))
                ToolsReport.Add(new Export());

            if (await IPermiso.ConsultaEsAsignadoPorSesion(ContratoAcceso.Imprimir, Empresa.Codigo))
            ...
            if (Contrato is null)
            {
                ...
                Notify.Show("El registro del contrato consultado a ver reporte no está disponible", "error");
                return;
            }
            ReportSource = new
            (
