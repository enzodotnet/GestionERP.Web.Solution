...
public partial class Report : IDisposable
{
    ...
    private const string nombreReporte = "SolicitudServicioReport.trdp";
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(SolicitudAcceso.VerReporte, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para ver reporte de [Solicitudes de Servicio]", "error");
				return;
			}
            ...
            if (Solicitud is null)
            {
                ...
                Notify.Show($"El registro de la [Solicitud de Servicio] consultado a ver reporte no está disponible", "error");
                return;
            }
            ReportSource = new
            (
    ...
    { 
        try
        {
            ...
            Notify.Show($"La solicitud de servicio {Solicitud.Codigo} ha sido impresa con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
