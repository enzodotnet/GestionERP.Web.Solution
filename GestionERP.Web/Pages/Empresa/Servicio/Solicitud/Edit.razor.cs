...
public partial class Edit : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(SolicitudAcceso.Editar, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para editar registros de [Solicitudes de Servicio]", "error");
				return;
			}
            ...
            if (Solicitud is null || Solicitud.CodigoEstado is not "ED")
            {
                ...
                Notify.Show($"El registro de la [Solicitud de Servicio] consultado a editar no está disponible", "error");
                return;
            }
    ...
    private async Task Editar()
    {
        try
        {
            ...
            IsModified = false;
            Notify.Show($"La solicitud de servicio {Solicitud.Codigo} ha sido editada con éxito", "success");
    ...
    private bool VerificarRegistroEsValido()
    {
        ...
        if (string.IsNullOrEmpty(SolicitudEditar.CodigoEntidad) && string.IsNullOrEmpty(SolicitudEditar.ReferenciaProveedor))
        {
        ...
        {
            Fnc.MostrarAlerta(Alert, "Es obligatorio que agregue ítem(s) al detalle de la solicitud de servicio", "error");
            esValido = false;
        }
        else 
        {
    ...
    private void OnChangeDetalleCantidadHandler(object value) => IsModifiedDetalle = Fnc.VerifyContextIsChanged(Detalle.Cantidad != (int?)value, TipoAccionModal is "E" ? DetalleEditarContext : DetalleInsertarContext, "Cantidad");
    ...
    {
        SolicitudEditar.CodigoLocalAtencion = Solicitud.CodigoLocalAtencion = item.CodigoLocal;
	...
	private async Task OnChangeEntidadHandler(object value)
	{
		...
		{ 
			SolicitudEditar.CodigoEntidad = string.IsNullOrEmpty(codigo) ? null : codigo;
    ...
    {
        SolicitudEditar.CodigoLocalAtencion = value?.Trim().ToUpper();
        if (!string.IsNullOrEmpty(Validator.MsgErrorLocalAtencion) && (Solicitud.CodigoLocalAtencion ?? "").Trim() != (SolicitudEditar.CodigoLocalAtencion ?? ""))
	...
	private async Task OnChangeLocalHandler(object value)
	{
		...
		{ 
			SolicitudEditar.CodigoLocalAtencion = string.IsNullOrEmpty(codigo) ? null : codigo; 
            if ((Solicitud.CodigoLocalAtencion ?? "") != codigo)
			{
                IsChangedLocal = true;
                if (!string.IsNullOrEmpty(codigo))
                {
    ...
    private void OnChangeReferenciaProveedorHandler(object value) => NotifyChange(Fnc.VerifyContextIsChanged((Solicitud.ReferenciaProveedor ?? "") != ((string)value ?? ""), EditContext, "ReferenciaProveedor"));
	...
	private void OnChangeFechaAtencionHandler(object value) => NotifyChange(Fnc.VerifyContextIsChanged(Solicitud.FechaAtencion != (DateTime?)value, EditContext, "FechaAtencion"));
	#endregion
