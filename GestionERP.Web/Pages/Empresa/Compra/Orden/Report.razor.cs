    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(OrdenAcceso.VerReporte, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para ver reporte de [Órdenes de Compra]", "error");
				return;
			}
            ...
            if (Orden is null)
            {
                ...
                Notify.Show($"El registro de la [Orden de Compra] consultado a ver reporte no está disponible", "error");
                return;
            } 
            ReportSource = new
            (
