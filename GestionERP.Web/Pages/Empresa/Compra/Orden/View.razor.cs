...
public partial class View : IDisposable
{
    ...
    private const string nombreReporte = "OrdenCompraReport.trdp";
	...
	private bool EsVisibleEnviarIngreso { get; set; }
    private bool EsAsignadoRevertirEnvioIngreso { get; set; }
    private bool EsVisibleRevertirEnvioIngreso { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            ...
            if (Orden is null)
            {
                ...
                Notify.Show($"El registro de la [Orden de Compra] a visualizar no está disponible", "error");
                return;
            }
    ...
    private async Task Accionar()
    {
        try
        {
            ...
            Notify.Show($"La orden de compra {Orden.Codigo} ha sido {ResultadoAccion} con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private async Task ImprimirDirecto()
    {
        try
        {
            ...
            Notify.Show($"La orden de compra {Orden.Codigo} ha sido impresa con éxito", "success");
        }
        catch (Exception ex)
        {
            if (ex is HttpRequestException)
    ...
    private void ValidarEstadoOptionButtons(string codigoEstado)
    {
		...
		EsVisibleEnviarIngreso = codigoEstado is "AP" && Orden.FlagEstadoIngreso is "NI";
        EsVisibleRevertirEnvioIngreso = codigoEstado is "AP" && Orden.FlagEstadoIngreso is "PI";
    ...
    private void MostrarAccionDialog(string codigoEstado = "")
    {
        ...
        IconoAccionModal = CodigoEstadoAccion switch
        {
            ...
            "EI" or "EA" => SvgIcon.RotateRight,
            "RI" or "RC" => SvgIcon.RotateLeft,
            _ =>  SvgIcon.Trash
        };
