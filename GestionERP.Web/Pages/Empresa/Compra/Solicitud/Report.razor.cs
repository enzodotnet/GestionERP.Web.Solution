    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(SolicitudAcceso.VerReporte, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para ver reporte de [Solicitudes de Compra]", "error");
				return;
			}
            ...
            if (Solicitud is null)
            {
                ...
                Notify.Show($"El registro de la [Solicitud de Compra] consultado a ver reporte no está disponible", "error");
                return;
            } 
            ReportSource = new
            (
