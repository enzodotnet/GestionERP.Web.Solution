    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
			...
			if (!await IPermiso.ConsultaEsAsignadoPorSesion(SolicitudAcceso.Editar, Empresa.Codigo))
			{
				...
				Notify.Show("No tiene permiso para editar registros de [Solicitudes de Compra]", "error");
				return;
			}
            ...
            if (Solicitud is null || Solicitud.CodigoEstado is not "ED")
            {
                ...
                Notify.Show($"El registro de la [Solicitud de Compra] consultado a editar no está disponible", "error");
                return;
            }
    ...
    private bool VerificarRegistroEsValido()
    {
        ...
        else 
        {
            ...
            else
            {
                ...
                {
                    Fnc.MostrarAlerta(Alert, "Está pendiente completar alguna celda de [cantidad] del detalle de la solicitud de compra", "error");
                    esValido = false;
                } 
            }
        } 
