...
public class PedidoInsertarDto
{
    ...
    public List<PedidoDetalleInsertarDto> Detalles { get; set; } = [];
}

public class PedidoInsertarValidator : AbstractValidator<PedidoInsertarDto>
{
    ...
    public PedidoInsertarValidator()
    {
        RuleFor(p => p.CodigoDocumento).NotEmpty().WithMessage("Es necesario que seleccione la serie del documento");
        
        RuleFor(p => p.CodigoOrdenReferencia).NotEmpty().WithMessage("El documento de la orden de importación es requerido"); 
