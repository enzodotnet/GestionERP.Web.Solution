...
public class ProduccionSolicitudApi(HttpClient httpClient) : IProduccionSolicitud
{ 
    protected ErrorEndpointResponse error = new();
    private const string pathApi = "empresa/{ce}/produccion/solicitudes";

    public async Task<IEnumerable<SolicitudListarDto>> Listar(string codigoEmpresa, string codigoEjercicio, string codigoPeriodo = null, string flagTipoRegistro = null)
    {
        try
        { 
    ...
    public async Task<Guid> Insertar(string codigoEmpresa, SolicitudInsertarDto solicitud)
    {
        try
        {
            using HttpResponseMessage response = await httpClient.PostAsJsonAsync(pathApi.Replace("{ce}", codigoEmpresa), solicitud); 
            if (response.IsSuccessStatusCode)
            {
    ...
    public async Task Editar(string codigoEmpresa, Guid id, SolicitudEditarDto solicitud)
    {
        try
        {
            using HttpResponseMessage response = await httpClient.PutAsJsonAsync($"{pathApi.Replace("{ce}", codigoEmpresa)}/{id}", solicitud);
            if (!response.IsSuccessStatusCode)
            {
                error = response.StatusCode == HttpStatusCode.NotFound ? new(){ Code = "NF" } : await response.Content.ReadFromJsonAsync<ErrorEndpointResponse>();
                throw new HttpResponseException(error.Message, error.Code);
    ...
    public async Task<IEnumerable<SolicitudCatalogoAtenderDto>> CatalogoAtender(string codigoEmpresa, string codigoEjercicio, string codigoProcesoDocumento, string codigoLocal = null)
    {
        ...
        { 
            ...
            using HttpResponseMessage response = await httpClient.GetAsync(QueryHelpers.AddQueryString($"{pathApi.Replace("{ce}", codigoEmpresa)}/catalogo/atender", query)); 
            if (response.IsSuccessStatusCode)
            {
                if (response.StatusCode == HttpStatusCode.NoContent)
                    return default;
